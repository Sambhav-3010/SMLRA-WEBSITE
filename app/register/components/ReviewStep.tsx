...
import { CheckCircle, AlertCircle, FileText, User, GraduationCap, Target } from 'lucide-react'
  ...
  const renderField = (label: string, value: string, required: boolean = false) => {
    ...
    return (
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start py-2 gap-1 sm:gap-4">
        ...
        <div className="w-full sm:max-w-xs text-left sm:text-right break-words break-all">
          {value ? (
            isUrl ? (
              <a 
                href={value} 
