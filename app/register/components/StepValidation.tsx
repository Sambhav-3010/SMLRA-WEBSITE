...
export const VALIDATION_ERRORS: Record<keyof FormData, Record<string, string>> = {
  firstName: {
...
export const VALIDATION_RULES: Record<keyof FormData, {
  required?: boolean
  minLength?: number
  maxLength?: number
  pattern?: RegExp
...
export const validateField = (field: keyof FormData, value: string): string[] => {
  ...
  if (value && value.trim() !== '') {
    ...
    if (rules.minLength && value.trim().length < rules.minLength) {
      errors.push(VALIDATION_ERRORS[field]?.minLength || `${field} must be at least ${rules.minLength} characters`)
    }

    // Max length validation
    if (rules.maxLength && value.trim().length > rules.maxLength) {
      errors.push(VALIDATION_ERRORS[field]?.maxLength || `${field} must be less than ${rules.maxLength} characters`)
    }

    // Pattern validation
    if (rules.pattern && !rules.pattern.test(value.trim())) {
      errors.push(VALIDATION_ERRORS[field]?.pattern || `${field} format is invalid`)
    }
  }
...
export const getStepErrors = (requiredFields: (keyof FormData)[], formData: FormData): Record<string, string[]> => {
...
export default function StepValidation({ step, requiredFields, onValidationChange }: StepValidationProps) {
